...
namespace MSRA.SpringField.Application
{
    public partial class EmploymentDecision : System.Web.UI.Page
    {
        ...
        {
            if (ddlStatus.SelectedValue == Convert.ToInt32(InterviewStatusEnum.Hired).ToString())//hired
            {
            ...
            else if (ddlStatus.SelectedValue == Convert.ToInt32(InterviewStatusEnum.Rejected).ToString())//rejected
            {
        ...
        protected string ParseDecision(object dataItem)
        {
            ...
            int approvalResult = Convert.ToInt32(dr["InterviewStatus"]);
        ...
        {
            DataRowView dr = dataItem as DataRowView;
            if (DateTime.Parse(dr["EndDate"].ToString()).Equals(DateTime.Parse("12/31/9999 11:59:59 PM")))
            {
                return "N/A";
            }
            else
