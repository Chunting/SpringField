...
public class NewHirePackage : IHttpHandler {
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        if (InterviewNum > 0)
        {
            if (!String.IsNullOrEmpty(dsInternview.Tables[0].Rows[InterviewNum - 1]["GMApprovalDocId"].ToString()) && Convert.ToInt32(dsInternview.Tables[0].Rows[InterviewNum - 1]["GMApprovalDocId"].ToString()) > 0)
            {
                ...
                string ApprovalPath = Springfield.Components.Configuration.SiteConfiguration.GetConfig().SiteAttributes["docPath"] + doc.SaveName;
    ...
    {
        string docPath = Springfield.Components.Configuration.SiteConfiguration.GetConfig().SiteAttributes["docPath"];
