...
namespace MSRA.SpringField.Application
{
    public partial class MentorPA : System.Web.UI.Page
    {
        ...
        protected void btnSaveEdittingInternPA_Click(object sender, EventArgs e)
        {
            ...
            if (!PA.Update())
            {
                JSUtility.Alert(this.Page, "Sorry, failed to save, please try again");
                return;
            }
        ...
        protected void btnSummit_Click(object sender, EventArgs e)
        {
            if (Page.IsValid)
            {
                string tbCommentsText = tbComments.Text;
                MatchCollection mc = System.Text.RegularExpressions.Regex.Matches(tbCommentsText, @"\b([a-zA-Z]+)\b|([a-zA-Z]+)");
                if (mc.Count < 80)
                {
                    LabelWordsNotice.Text = "At least 80 words! Current Words=" + mc.Count;
                    return;
                }
                else
                {
                    LabelWordsNotice.Text = "";
                }
                ...
                if (result != null)
                {
                    ...
                    {
                        JSUtility.Alert(this.Page, "Sorry,failed to submit this performance assessment, please try again ");
                    }
                }
                else
                {
        ...
        protected void btnSummitByUR_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(rblCodingskill.SelectedValue))
            {
                PA.CodingSkill = Convert.ToInt32(rblCodingskill.SelectedValue);
            }
            if (!string.IsNullOrEmpty(rblAnalyticalSkill.SelectedValue))
            {
                PA.AnalyticalSkill = Convert.ToInt32(rblAnalyticalSkill.SelectedValue);
            }
            if (!string.IsNullOrEmpty(rblProblemSolving.SelectedValue))
            {
                PA.ProblemSolving = Convert.ToInt32(rblProblemSolving.SelectedValue);
            }
            if (!string.IsNullOrEmpty(rblInnovation.SelectedValue))
            {
                PA.Innovation = Convert.ToInt32(rblInnovation.SelectedValue);
            }
            if (!string.IsNullOrEmpty(rblDrivingforResults.SelectedValue))
            {
                PA.DrivingForResults = Convert.ToInt32(rblDrivingforResults.SelectedValue);
            }
            if (!string.IsNullOrEmpty(rblDealingwithAmbiguity.SelectedValue))
            {
                PA.DealingWithAmbiguity = Convert.ToInt32(rblDealingwithAmbiguity.SelectedValue);
            }
            if (!string.IsNullOrEmpty(rblQuickonLearning.SelectedValue))
            {
                PA.QuickOnLearing = Convert.ToInt32(rblQuickonLearning.SelectedValue);
            }
            if (!string.IsNullOrEmpty(rblEnglish.SelectedValue))
            {
                PA.English = Convert.ToInt32(rblEnglish.SelectedValue);
            }
            if (!string.IsNullOrEmpty(rblCommunicationSkills.SelectedValue))
            {
                PA.Communication = Convert.ToInt32(rblCommunicationSkills.SelectedValue);
            }
            if (!string.IsNullOrEmpty(rblTeamWork.SelectedValue))
            {
                PA.TeamWork = Convert.ToInt32(rblTeamWork.SelectedValue);
            }
            if (!string.IsNullOrEmpty(rblAttitude.SelectedValue))
            {
                PA.Attitude = Convert.ToInt32(rblAttitude.SelectedValue);
            }
            if (!string.IsNullOrEmpty(rblIntegrityandHonesty.SelectedValue))
            {
                PA.IntegrityHonesty = Convert.ToInt32(rblIntegrityandHonesty.SelectedValue);
            }
            if (!string.IsNullOrEmpty(rblOpenandRespectful.SelectedValue))
            {
                PA.OpenRespectful = Convert.ToInt32(rblOpenandRespectful.SelectedValue);
            }
            if (!string.IsNullOrEmpty(rblBigChallenges.SelectedValue))
            {
                PA.BigChallenges = Convert.ToInt32(rblBigChallenges.SelectedValue);
            }
            if (!string.IsNullOrEmpty(rblPassion.SelectedValue))
            {
                PA.Passion = Convert.ToInt32(rblPassion.SelectedValue);
            }
            if (!string.IsNullOrEmpty(rblAccountable.SelectedValue))
            {
                PA.Accountable = Convert.ToInt32(rblAccountable.SelectedValue);
            }
            if (!string.IsNullOrEmpty(rblSelfCritical.SelectedValue))
            {
                PA.SelfCritical = Convert.ToInt32(rblSelfCritical.SelectedValue);
            }
            if (!string.IsNullOrEmpty(rblOverallEvaluation.SelectedValue))
            {
                PA.OverrallEvaluation = Convert.ToInt32(rblOverallEvaluation.SelectedValue);
            }
